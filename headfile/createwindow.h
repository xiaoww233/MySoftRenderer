...
#define IDM_SETTING_BCULLING 1004
...
HMENU createmianmenu() {
	...
	AppendMenu(hsettingmenu, MF_STRING | MF_CHECKED, IDM_SETTING_BCULLING, L"背面剔除");
	AppendMenu(hmenu, MF_POPUP, (UINT_PTR)hfilemenu, L"文件");
	AppendMenu(hmenu, MF_POPUP, (UINT_PTR)hsettingmenu, L"设置");
	return hmenu;
}
...
void UpdateBackfaceCullingMenu(HMENU hMenu, bool bculling)
{
	MENUITEMINFO mii = { 0 };
	mii.cbSize = sizeof(MENUITEMINFO);
	mii.fMask = MIIM_STATE;
	mii.fState = bculling ? MFS_CHECKED : MFS_UNCHECKED;

	SetMenuItemInfo(hMenu, IDM_SETTING_BCULLING, FALSE, &mii);
}
